...
<%		
		...
		try {
			...
			String sqlHit = " UPDATE free_board"
				     	  + " SET hit = hit + 1"
				     	  + " WHERE fno = ?";
			...
			String sql = "SELECT u.uno"
					   ...
					   + " , hit"
					   + " , content"
					   + " , (select count(*) from f_comment c where c.state = 'E' AND fno = ?) as cnt"
					   + " FROM free_board f"
					   + " INNER JOIN user u"
					   + " ON f.uno = u.uno"
					   + " WHERE fno = ?";
			...
			psmtCommentPaging.setInt(1, fno);
...
%>
									<form action="modify.jsp?fno=<%= fno %>" method="post">
			                        	<button>수정</button>
			                    	</form>
<%									
								}
...
%>		                
							                    <a href="<%=request.getContentType()%>/FreeBoard/view.jsp?fno=<%=fno%>&nowPage=<%=paging.getStartPage()-1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">이전</a>
<%							
											}
...
%>
													<a href="<%=request.getContextPath()%>/FreeBoard/view.jsp?fno=<%=fno%>&nowPage=<%= i %>&searchType=<%=searchType%>&searchValue=<%=searchValue%>"><%= i %></a>
<%								
												}
											}
...
%>
												<a href="<%=request.getContextPath()%>/FreeBoard/view.jsp?fno=<%=fno%>&nowPage=<%=paging.getStartPage()+1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">다음</a>
<%														
									}
								}
%>
