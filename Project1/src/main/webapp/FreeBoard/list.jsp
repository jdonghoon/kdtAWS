...
<%		
		...
		String searchValue = request.getParameter("freeboard_text");
		...
		try {
			...
			String sql = " SELECT fno"
			   ...
			   + " , (select count(*) from f_comment c where c.fno = f.fno AND c.state = 'E') as cnt"
			   + " , hit"
			   + " FROM free_board f"
			   + " INNER JOIN user u"
			   + " ON f.uno = u.uno"
			    ...
			    <body>
			            ...
			            <nav>
			                <div class="navbar">
			                    <ul>
			                        ...
			                        <li><a href="list.jsp" class="navmenu" style="font-weight: bold;">FreeBoard</a></li>
			        ...
			        <section>
			            <article>
			                ...
			                <div class="search_freeboard">
			                    <form action="<%=request.getContextPath()%>/FreeBoard/list.jsp?searchType=<%=searchType%>&searchValue=<%=searchValue %>" method="get">
			                        ...
			                        <input type="text" name="freeboard_text" value="<%=searchValue%>">
			                        <button>검 색</button>
			                    </form>
			                </div>
			                <form action="<%=request.getContextPath()%>/FreeBoard/register.jsp" method="post">
                		...
                		<%
							...
							if(total == 0) {
						%>
							<div class="freeboard_none">게시글이 없습니다.</div>
						<%						
							} else {
			                	while(rs.next()) {
			                		count++;
			                		...
			                		hit = rs.getInt("hit");
			                %>
			                	<ul class="freeboard_list">
				                    <li class="no"><div style="width: 50px;"><%= (paging.getNowPage() > 1) ? ((paging.getNowPage()-1)*10) + count : count %></div></li>
				                    <li class="title">
				                    	<a href="<%=request.getContextPath()%>/FreeBoard/view.jsp?fno=<%= fno %>">
				                    		<div>
				                    			[<%= brand %>] <%= title %><span style="color: orange; font-weight: bold"> [<%=cnt%>]</span>
				                    		</div>
				                    	</a>
				                    </li>
							...
							%>		                
					                    <a href="<%=request.getContentType()%>/FreeBoard/list.jsp?nowPage=<%=paging.getStartPage()-1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">이전</a>
							<%							
									}
							...
							%>
											<a href="<%=request.getContextPath()%>/FreeBoard/list.jsp?nowPage=<%= i %>&searchType=<%=searchType%>&searchValue=<%=searchValue%>"><%= i %></a>
							<%								
										}
									}
							...
							%>
										<a href="<%=request.getContextPath()%>/FreeBoard/list.jsp?nowPage=<%=paging.getEndPage()+1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">다음</a>
							<%														
									}
								}
							%>
